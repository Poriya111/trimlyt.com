...
router.put('/:id', auth, async (req, res) => {
    const { service, price, date, extras, status, needsCompletion } = req.body;
    ...
    if (typeof needsCompletion !== 'undefined') appointmentFields.needsCompletion = needsCompletion;
...
router.post('/sync-google', auth, async (req, res) => {
    try {
        ...
        // Get keyword from request, default to 'TL'
        const { importKeyword } = req.body;
        const keyword = importKeyword && importKeyword.trim() ? importKeyword.trim() : 'TL';
        ...
        // Regex for full format: Keyword Service Price (e.g., "TL Haircut 25")
        const fullPattern = new RegExp(`^${escapedKeyword}\\s+(.+?)\\s+(\\d+(?:\\.\\d{1,2})?)$`, 'i');
        
        // Regex for prefix match (e.g., "TL ...")
        const prefixPattern = new RegExp(`^${escapedKeyword}(?:\\s+|$)`, 'i');

        for (const event of events) {
            if (!event.start || (!event.start.dateTime && !event.start.date)) continue;
            
            const summary = event.summary || '';
            let service, price, needsCompletion = false;

            // 1. Try full pattern first
            const fullMatch = summary.match(fullPattern);
            if (fullMatch) {
                service = fullMatch[1].trim();
                price = parseFloat(fullMatch[2]);
            } else if (prefixPattern.test(summary)) {
                // 2. Check if it matches the keyword prefix (Partial match)
                const remainder = summary.replace(new RegExp(`^${escapedKeyword}\\s*`, 'i'), '').trim();
                service = remainder || summary;
                price = 0;
            ...
            if (!existing) {
                ...
                await newApp.save();
                importedCount++;
                console.log(`Imported: ${service} ($${price}) on ${appointmentDate}`);
            }
        }
